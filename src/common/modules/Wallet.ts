...
export class Wallet {
    ...
    /**
     * @description Instantiates new Wallet with params
     * @param {string} type
     * @param {string} name
    ...
    static create(
        ...
        if (!hash) {
            thisWallet.hash = Hex.stringify(sha256(`${name} + ${type}`))
        } else {
            thisWallet.hash = hash
        }

        thisWallet.balances.USDC = parseFloat(`${usdcBalance}`)
        thisWallet.initialBalance = parseFloat(`${usdcBalance}`)
        thisWallet.type = type
        thisWallet.name = name
        thisWallet.default = isDefault
    ...
    createQuest(name) {
        ...
        this.questsCreated.push(quest.hash)
        return quest
    }
    ...
    private modifyPosition(
        ...
        pool.questLeftVolume -= amounts[0]
        pool.questRightVolume -= amounts[1]
