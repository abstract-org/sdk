...
export const formSwapData = (pool, investor, action, totalAmountIn, totalAmountOut, paths, day, opName) => {
    return {
        pool: pool ? pool.name : '',
...
const _resetObjProps = (propsToReset, resetValue = '') => (obj) => {
    return Object.entries(obj).reduce((resultObj, [k, v]) => ({
        ...resultObj,
...
export const sanitizeRemoteScenario = (loadedObj) => ({
    ...
    scenario: {
        ...loadedObj.scenario,
        invConfigs: loadedObj.scenario.invConfigs.map(_resetObjProps(['excludeSingleName', 'includeSingleName'], '')),
        questConfigs: loadedObj.scenario.questConfigs.map(_resetObjProps(['citeSingleName'], ''))
    }
});
...
export const isNearZero = (amount) => {
    return (0 ===
        parseInt(amount.toFixed(10).replace(/\D/g, '').split('').slice(0, 8).join('')));
};
...
export const getPathActions = (path, router) => {
    ...
    for (const [id, quest] of path.entries()) {
        ...
        else if (quest === curPool.tokenLeft ||
...
export const hashmapToObj = (hm) => hm.entries().reduce((o, [k, v]) => ({ ...o, [k]: v }), {});
 ...
 * @returns {HashMap}
 */
export function createHashMappings(arr, linkingKey, linkingValue) {
...
export function convertArrayToHashMapByKey(arr, key) {
    const resultHashMap = new HashMap();
    for (const item of arr) {
        if (item[key] != null)
            resultHashMap.set(item[key], item);
    }
...
export const inRangeFilter = (range) => (day) => {
    ...
    const isGreaterOrEqualFrom = !Number.isFinite(from) || Number.isNaN(from) || day >= from;
    const isLessOrEqualTo = !Number.isFinite(to) || Number.isNaN(to) || day <= to;
