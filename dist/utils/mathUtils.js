...
export const buySameLiqGiveT1GetT0 = (liq, price, amount) => {
    return Math.abs((liq * price * amount) / (liq + Math.sqrt(price) * (amount * -1)));
};
/**
...
export const buySameLiqGiveT0GetT1 = (liq, price, amount) => {
    return Math.abs(liq * (1 / (Math.sqrt(price) + amount / liq) - 1 / Math.sqrt(price)));
};
/**
...
export const sellSameLiqGiveT1GetT0 = (liq, price, amount) => {
    return Math.abs(liq * (Math.sqrt(price) - liq / (amount + liq / Math.sqrt(price))));
};
