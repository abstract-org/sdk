export class Graph {
    ...
    buildPathwaysUtil(tokenA, tokenB, isVisited, localPathList) {
        if (tokenA === tokenB) {
            let path = [];
            localPathList.forEach((node) => node !== 'undefined' ? path.push(node) : null);
        ...
        for (let i = 0; i < this.adjList.get(tokenA).length; i++) {
            if (!isVisited[this.adjList.get(tokenA)[i]] &&
                ...
                this.buildPathwaysUtil(this.adjList.get(tokenA)[i], tokenB, isVisited, localPathList);
                // remove current node
                // in path[]
                localPathList.splice(localPathList.indexOf(this.adjList.get(tokenA)[i]), 2);
            }
        }
        // Mark the current node
