...
export class Pool {
     ...
     */
    openPosition(priceMin, priceMax, token0Amt = 0, token1Amt = 0, native = false) {
        let amounts = [];
        let amount1 = 0;
        ...
        const liquidity = this.getLiquidityForAmounts(token0Amt, token1Amt, Math.sqrt(priceMin), Math.sqrt(priceMax), Math.sqrt(this.curPrice));
        if (liquidity === 0) {
            return [];
        }
        ...
        amounts = this.getAmountsForLiquidity(liquidity, Math.sqrt(priceMin), Math.sqrt(priceMax), Math.sqrt(this.curPrice));
        if (amount1 > 0) {
            amounts[1] = amount1;
        }
    ...
    setActiveLiq(pMin, native) {
        if (pp2p(this.curPP) !== this.curPrice ||
            ...
            const ppNext = pMin <= this.curPrice
     ...
     * @returns {number}
     */
    getLiquidityForAmounts(token0, token1, sqrtPriceMin, sqrtPriceMax, currentSqrtPrice) {
    ...
    buy(amount, priceLimit = null, dry = false) {
        let totalAmountIn = 0, totalAmountOut = 0;
        let nextPricePoint;
        let nextPriceTarget;
        let curLiq;
        ...
        do {
            ...
            journal[i].push(`Current left/right: ${this.curLeft}/${this.curRight}`);
            journal[i].push('---');
            journal[i].push(`Next price point: log2(${nextPricePoint}) dec(${pp2p(nextPricePoint)})`);
            journal[i].push(`Next price shift: ${nextPriceTarget} (${Math.sqrt(nextPriceTarget)})`);
            ...
            journal[i].push('---');
            if (Math.sqrt(nextPriceTarget) >= Math.sqrt(pp2p(nextPricePoint)) &&
                arrivedAtSqrtPrice >= Math.sqrt(nextPriceTarget)) {
                journal[i].push(`Arrived price is >= than ${arrivedAtSqrtPrice >= Math.sqrt(nextPriceTarget)
                    ? 'nextPriceTarget'
                    : 'nextPricePoint'}, capping at ${arrivedAtSqrtPrice}`);
                ...
                journal[i].push(`Next left/right: ${this.curLeft}/${this.curRight}`);
                journal[i].push('!!! ---');
            ...
            journal[i].push(`Calculating Amount0: ${curLiq} * ${arrivedAtSqrtPrice} - ${Math.sqrt(this.curPrice)}`);
            let amount0UntilNextPrice = curLiq * (arrivedAtSqrtPrice - Math.sqrt(this.curPrice));
            journal[i].push(`Calculating Amount1: ${curLiq} * 1/${arrivedAtSqrtPrice} - 1/${Math.sqrt(this.curPrice)}`);
            journal[i].push('---');
            let amount1UntilNextPrice = curLiq * (1 / arrivedAtSqrtPrice - 1 / Math.sqrt(this.curPrice));
            journal[i].push(`Amount0 (curLiq * (arrived - sqrt(curPrice)):\n ${amount0UntilNextPrice}`);
            journal[i].push(`Amount1 (curLiq * (1/arrivedAtSqrtPrice - 1/sqrt(curPrice))):\n ${amount1UntilNextPrice}`);
            journal[i].push('---');
            this.curPrice = arrivedAtSqrtPrice ** 2;
            this.priceToken1 = this.curPrice;
            this.priceToken0 = 1 / this.curPrice;
            journal[i].push(`New price: ${this.curPrice} (${arrivedAtSqrtPrice})`);
            if (amount1UntilNextPrice === -Infinity ||
            ...
            journal[i].push(`Remaining amount after paying ${amount0UntilNextPrice}: ${amount}`);
    ...
    sell(amount, priceLimit = null, dry = false) {
        ...
        while (amount > 0 &&
            ...
            journal[i].push(`Amount: ${amount}`);
            journal[i].push(`Bottom (current for buy) price point: ${this.curPP} (${pp2p(this.curPP)})`);
            journal[i].push(`Current price: ${this.curPrice} (sqrt ${Math.sqrt(this.curPrice)})`);
            journal[i].push(`Current liquidity: ${curLiq}`);
            journal[i].push(`Current left/right: ${this.curLeft} (${pp2p(this.curLeft)})/${this.curRight} (${pp2p(this.curRight)})`);
            journal[i].push('---');
            journal[i].push(`Next price point: ${nextPricePoint} (${pp2p(nextPricePoint)})`);
            journal[i].push(`Next price shift: ${nextPriceTarget}`);
            journal[i].push('---');
            journal[i].push(`Arrived price before cap: ${arrivedAtSqrtPrice} (${arrivedAtSqrtPrice ** 2})`);
            if (arrivedAtSqrtPrice < Math.sqrt(nextPriceTarget)) {
                arrivedAtSqrtPrice = Math.sqrt(nextPriceTarget);
            }
            journal[i].push(`Arrived price after cap: ${arrivedAtSqrtPrice} (${arrivedAtSqrtPrice ** 2})`);
            journal[i].push('---');
            if (Math.sqrt(nextPriceTarget) <= Math.sqrt(pp2p(nextPricePoint)) &&
                arrivedAtSqrtPrice <= Math.sqrt(nextPriceTarget)) {
                journal[i].push(`Arrived price is <= than ${arrivedAtSqrtPrice <= Math.sqrt(nextPriceTarget)
                    ? 'nextPriceTarget'
                    : 'nextPricePoint'}, capping at ${arrivedAtSqrtPrice} (${arrivedAtSqrtPrice ** 2})`);
                const np = this.pos.get(nextPricePoint);
                journal[i].push(`Next position: liq: ${np.liquidity} left: ${np.left} pp: ${np.pp} right: ${np.right}`);
                ...
                journal[i].push(`Next liquidity: ${this.curLiq}`);
                journal[i].push(`Next left/right: ${this.curLeft} (${pp2p(this.curLeft)})/${this.curRight} (${pp2p(this.curRight)})`);
                journal[i].push('!!! ---');
                journal[i].push(`WILL MOVE TO NEXT POSITION: ${nextPricePoint}`);
                journal[i].push('!!! ---');
            }
            journal[i].push(`Calculating amount0: ${curLiq} * (1 / ${Math.sqrt(this.curPrice)} - (1 / ${arrivedAtSqrtPrice})`);
            let amount0UntilNextPrice = curLiq * (1 / Math.sqrt(this.curPrice) - 1 / arrivedAtSqrtPrice);
            journal[i].push(`Calculating amount1: ${curLiq} * (${Math.sqrt(this.curPrice)} - ${arrivedAtSqrtPrice})`);
            let amount1UntilNextPrice = curLiq * (Math.sqrt(this.curPrice) - arrivedAtSqrtPrice);
            journal[i].push('---');
            journal[i].push(`Amount0 (curLiq * (1 / sqrt(curPrice) - (1 / arrivedAt)): ${amount0UntilNextPrice}`);
            journal[i].push(`Amount1 (curLiq * (sqrt(curPrice) - arrivedAt)): ${amount1UntilNextPrice}`);
            journal[i].push('---');
            this.curPrice = arrivedAtSqrtPrice ** 2;
            this.priceToken1 = this.curPrice;
            this.priceToken0 = 1 / this.curPrice;
            journal[i].push(`New price: ${this.curPrice} (${Math.sqrt(arrivedAtSqrtPrice)})`);
            journal[i].push('---');
            amount += amount0UntilNextPrice;
            journal[i].push(`Remaining amount after subtracting ${amount0UntilNextPrice}: ${amount}`);
            totalAmountOut += Math.abs(amount1UntilNextPrice);
            totalAmountIn += amount0UntilNextPrice;
            journal[i].push(`Total in ${totalAmountIn} / total out ${totalAmountOut}`);
    ...
    getSwapInfo(logOut = false) {
        ...
        if (logOut) {
            ...
            buy(amt): ${this.tokenLeft} in (deduct from amt), ${this.tokenRight} out (add to balance)
            sell(amt): ${this.tokenRight} in (deduct from amt), ${this.tokenLeft} out (add to balance)
            --
            total ${this.tokenLeft}: ${rightBalance[1] > 0 ? rightBalance[1] : 0}
            total ${this.tokenRight}: ${leftBalance[1] > 0 ? leftBalance[1] : 0}
            ---
            can buy(take in) ${Math.abs(leftBalance[0])} ${this.tokenLeft} for(give out) ${Math.abs(leftBalance[1])} ${this.tokenRight}
            can sell(give) ${Math.abs(rightBalance[1])} ${this.tokenLeft} for(take) ${Math.abs(rightBalance[0])} ${this.tokenRight}`);
        }
    ...
    getMarketCap() {
        ...
        const totalTokens = this.posOwners.reduce((acc, val) => acc + val.amt1, 0);
