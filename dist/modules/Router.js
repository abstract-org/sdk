...
import { buySameLiqGiveT0GetT1, buySameLiqGiveT1GetT0, getSwapAmtSameLiq, sellSameLiqGiveT0GetT1, sellSameLiqGiveT1GetT0 } from '../utils/mathUtils';
...
export class Router {
    ...
    smartSwap(token0, token1, amountIn, smartRouteDepth = 3, forcedPath) {
        ...
        this._PAIR_PATHS[`${token0}-${token1}`] = this.calculatePairPaths(token0, token1, smartRouteDepth);
        ...
        do {
            this._PRICED_PATHS = this.drySwapForPricedPaths(this.getPairPaths(token0, token1));
            if (this._DEBUG)
                console.log('priced paths', this._PRICED_PATHS, `amountIn: ${amountIn}`);
            if (!this._PRICED_PATHS.length)
                return totalInOut;
        ...
        } while (this._PRICED_PATHS.length &&
            !isZero(amountIn) &&
    ...
    getMaxAmountInForPath(amountIn, path) {
        ...
        if (!pathActions || !pathActions.length)
            return 0;
        const pathWithActionsCaps = this.getPathWithActionCaps(pathActions);
        if (!pathWithActionsCaps || !pathWithActionsCaps.length)
            return 0;
        const maxAcceptable = this.calculateAcceptableForCappedPathActions(pathWithActionsCaps, path);
    ...
    getPathWithActionCaps(pathActions) {
        ...
        for (const step of pathActions) {
            ...
            const cappedAmountsSameLiq = getSwapAmtSameLiq(step.pool, zeroForOne);
            if (cappedAmountsSameLiq.t0fort1 === 0 ||
            ...
            if (idx > 0) {
                ...
                if (parseFloat(curSum) !== parseFloat(prevSum)) {
                    ...
                    if (isLeak) {
                        ...
                        console.error(`${path.join(' -> ')} traversal\n got ${prevSum} but passed further ${curSum}\n leaking [${prevSum - curSum}]`);
                        console.error('_PROTO_SWAPS:\n', [...this._PROTO_SWAPS].reverse());
                        console.error('pathSwaps:\n', pathSwaps);
                        console.error('With calculated path amounts:\n', pathWithActionsCaps);
    ...
    findPoolsFor(tokenName, maxDepth, depth = 1) {
        ...
        results.forEach((res) => {
            const leftPools = this.findPoolsFor(res.tokenLeft, maxDepth, depth + 1);
            const rightPools = this.findPoolsFor(res.tokenRight, maxDepth, depth + 1);
