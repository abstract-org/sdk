...
export class Investor {
    ...
    addBalance(tokenName, balance, msg = null) {
        ...
        if (this.balances[tokenName] + balance < 0) {
            console.log(`You don't have ${balance} of ${tokenName} to spend, remaining amount is ${this.balances[tokenName]}`, msg);
            return false;
        }
    ...
    removeLiquidity(pool, priceMin, priceMax, amountLeft = 0, amountRight = 0) {
        return this.modifyPosition(pool, priceMin, priceMax, amountLeft, amountRight);
    }
    modifyPosition(pool, priceMin, priceMax, amountLeft = 0, amountRight = 0) {
        const liquidity = pool.getLiquidityForAmounts(amountLeft, amountRight, Math.sqrt(priceMin), Math.sqrt(priceMax), Math.sqrt(pool.curPrice));
        ...
        const amounts = pool.getAmountsForLiquidity(liquidity, Math.sqrt(priceMin), Math.sqrt(priceMax), Math.sqrt(pool.curPrice));
    ...
    citeQuest(crossPool, priceMin = 1, priceMax = 10, token0Amt = 0, token1Amt = 0, native = false) {
        ...
        if (typeof token0Amt === 'undefined' ||
            ...
            console.warn(`During citation at ${crossPool.name} with pos[${priceMin}...${priceMax}] direction native=${native}, tokens passed token0: ${token0Amt}, token1: ${token1Amt}`);
     ...
     */
    calculatePriceRange(crossPool, citedQuestPool, citingQuestPool, multiplier = this.PRICE_RANGE_MULTIPLIER) {
        ...
        console.assert(min < max, 'priceMin (%s) is higher than priceMax (%s), skipping position opening for %s, unit price %s, direction native=%s', min, max, crossPool.name, unitPrice, nativePos);
        if (min > max) {
